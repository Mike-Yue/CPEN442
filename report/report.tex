...
\begin{abstract}
This paper seeks to address the problems that plague conventional keypad door locks. Firstly, a security investigation into this type of keypad lock was completed. This highlighted the innate vulnerabilities of a traditional static PIN entry system --- primarily replay attacks such as shoulder-surfing. Our proposed solution uses randomly generated One-Time passcodes together with user fingerprint biometrics to authenticate the user and produce an unlock code. A three-part prototype system was designed to demonstrate the increased security for a typical use-case scenario. User convenience and psychological acceptance were key considerations in our design. The system was then evaluated in usability studies in order to determine its overall effectiveness, and was shown to be a reasonable increase in security and a good starting point for future development. 
\end{abstract}
...
It can be hard to quantify or estimate the value of goods protected by electronic keypad locks. We postulate that if the location is important enough to secure with an electronic keypad lock, then the user does not want the location compromised, and thus security is important to the user. 
	...
	We designed a digital-lock system that employs the use of dynamically generated OTP (One-time passcode) access codes instead of conventional static codes. Our system uses fingerprint biometrics for authentication to complement logging in using a username and password --- a user will be able to authenticate with only biometrics if the application has not been closed (usually only with user action or a power cycle of the mobile device). Fingerprint authentication has been proven to be more usable and efficient than entering a username and password \cite{b2}, minimizing the amount of effort and time required on the user’s part. Once the user is authenticated, they can then generate an OTP for unlocking a particular digital door-lock. Upon unlocking the digital door-lock, the valid code is immediately destroyed and rendered invalid. 
	There has been similar work done by CMU on the Grey system \cite{b3}, a device-enabled authorization system utilizing smartphones to unify access controls. However, the system still utilizes a static PIN code in order to activate a user’s private key for authentication. This leaves the system still highly vulnerable to adversarial attacks such as shoulder surfing. Our proposed system thus hopes to remove the need for static PIN codes through OTP dynamic PIN codes. 
...
We concluded that while our prototype system was limited in scope, it served as a great foundation for a commercial security product. Our prototype was proven to defeat many common attacks against traditional keypad locks, as any type of replay attack was defeated by our single-use codes. 
...
\subsection{Overview}
	Instead of a static pin code, we use a dynamically-generated OTP access code. First, the user must login using a traditional username and password, once this has been done they should not need to re-enter their credentials unless the application is terminated. The server then sends the list of locks which the user is authorized to unlock, they can then select one of these options and attempt to generate an unlock code. Upon requesting this OTP, the application will prompt for authentication using the phone's fingerprint scanner. If this is successful a request will be sent to the server with the user's credentials.   User input in the lock is then sent to the server and verified on the server-side. If the code entered is valid, the server returns a success message and unlocks the lock. Otherwise, the server returns a forbidden message and the lock remains unlocked.
...
Moreover, adversaries may attempt to gain access to the access codes through a shoulder surfing attack. The system employs the use of expiring OTP codes that are valid only for a single-use. After which, the server immediately deletes the associated valid code. This means that even if an adversary were to successfully identify the user’s OTP code, it becomes useless upon use. This effectively nullifies the threat of adversarial replay attacks.

	Furthermore, an adversary may attempt to gain access to the authorized user’s device through a social engineering attack. Even if the attacker were to have unrestricted access to the user’s phone, they would not be able to generate a valid OTP without first authenticating themselves with their fingerprint biometrics. Communication with the server will only be established if the user’s identity is authenticated. This way, any requests made to the server to generate a OTP using our application can only be done by an authorized user.
...
Our system sought to strike a good balance between the extra security benefits of OTP codes and the convenience of a numeric keypad lock.
...
The current behaviour limits the effectiveness of the OTP codes and encourages users to generate codes ahead of time, making the system much less secure.
