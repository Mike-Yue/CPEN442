...
\begin{abstract}
This paper seeks to address the problems that plague conventional keypad door locks. Firstly, a security investigation into these type of keypad locks was completed. This highlighted the innate vulnerabilities of a traditional static PIN entry system --- primarily replay attacks such as shoulder-surfing. Our proposed solution uses dynamic, randomly generated Time-Based One-Time Password (TOTP) together with user fingerprint biometrics to generate a single-use PIN. A three-part prototype system was designed to demonstrate the increased authentication security for a typical use-case scenario. User convenience and psychological acceptance were key considerations in our design. The system was then evaluated in usability studies in order to access its overall effectiveness. For the prototype, the implementation was split into three parts: server-side (Mike), mobile application (Ben and Scott) and digital lock hardware (Austine).
\end{abstract}
